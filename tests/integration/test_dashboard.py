...
def test_dashboard_version_uses_central_source(monkeypatch, tmp_path):
    ...
    try:
        ...
        assert version["version"] == openapi["info"]["version"] == "1.0.0-beta.1"
        assert version["sprint"] == "40"
        assert version["migration"] in {"0024", "unknown"}
        assert "/api/platform/version" in script
        assert 'id="footer-version"' in html
        assert '<meta name="application-version" content="1.0.0-beta.1">' in html
